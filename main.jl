...
function train_from_scratch(;batch_size=2, num_epochs=30, learning_rate=1e-4)
    ...
    for epoch in 1:num_epochs
        ...
        if val_loss < best_val_loss
            ...
            Checkpoint.save_checkpoint("best_model.bson", model; epoch=epoch, loss=best_val_loss)
            model = to_device(model)
        end
    end
end

function train_from_checkpoint(checkpoint_path::String; batch_size=2, num_epochs=30, learning_rate=1e-4)
