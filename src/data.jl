...
function stft_mag(x::Vector{Float32}; n_fft::Int = N_FFT, hop_length::Int = HOP_LENGTH)
...
function griffin_lim(logmel_spec::Matrix{Float32}; n_iter::Int = 32)
    ...
    S_complex = S_linear .* exp.(im .* phase)
    frames = [irfft(S_complex[:, i], N_FFT) for i = 1:size(S_complex, 2)]
...
function list_librispeech_flac(root::AbstractString; split::String = "train-clean")
