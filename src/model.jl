...
function AudioEncoder(
    ...
    AudioEncoder(
        ...
        [TransformerBlock(embed_dim, heads, mlp_dim; dropout_rate) for _ in 1:depth],
        LayerNorm(embed_dim),
        Dropout(dropout_rate)
    )
end
...
function AudioDecoder(
    ...
    AudioDecoder(
        ...
        LayerNorm(embed_dim),
        Chain(
            Dense(embed_dim, embed_dim, gelu),
...
function ViTAudioCoder(;
    ...
    ViTAudioCoder(
        AudioEncoder(in_chans, embed_dim, enc_depth, heads, mlp_dim, patch_size, max_len; dropout_rate),
        Quantizer.VectorQuantizer(n_codes, embed_dim),
        AudioDecoder(embed_dim, dec_depth, heads, mlp_dim, out_chans, patch_size; dropout_rate)
    )
end
