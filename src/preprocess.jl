...
function to_patches(mel_fixed::AbstractMatrix{<:Real};
    ...
    idx = 1
    for fy in 0:(n_freq_patches-1)
        for tx in 0:(n_time_patches-1)
            f_start = fy * patch_freq + 1
            t_start = tx * patch_time + 1
            patch = mel_fixed[f_start:f_start+patch_freq-1,
                              t_start:t_start+patch_time-1]
